    ...
    <div className="max-w-[1200px] px-4 mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {skeletonItems.map((_, index) => (
          <div
            key={index}
            className="bg-[var(--color-bg)] rounded-2xl border border-[var(--color-border-secondary)] overflow-hidden"
          >
            <div className="relative">
              <div className="w-full h-[12rem] sm:h-[15rem] bg-[var(--color-border)] animate-pulse" />
              <div className="absolute top-2 left-2 h-6 w-16 bg-pink-200 rounded animate-pulse" />
            </div>

            <div className="py-4 px-2">
              <div className="h-4 w-3/4 bg-[var(--color-border)] rounded animate-pulse mb-2" />
